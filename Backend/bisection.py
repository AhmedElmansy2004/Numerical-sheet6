...
def do_bisection(func, from_val, to_val, precision, epsilon=0.00001, maxIterations=50):
    ...
    for i in range(maxIterations):
        # get the midpoint
        prev = x_r
        x_r = round_to_significant_digits(x_l + (x_u - x_l) / 2, precision)
...
if "__main__" == __name__:
    success, root, iterations, relative_error, precision, steps = do_bisection("sin(x)", 2, 4, 17, 0.0001, 100)
    # success, root, iterations, relative_error, precision, steps = do_bisection("x**2 - 9", 0, 4, 17, 0.0001, 100)
